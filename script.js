var searchString = "http://10.193.240.98/names?q=" + searchInput; 
...
function handleSearch(event){
    ...
    //It should make it so that only the squares containing text that match what the user typed in are displayed. 
    //Squares that do not contain text that match the user's input should be hidden.
...
// This function is called when the button labeled "Get Data" is pressed. 
//Its purpose is to fetch data from the server, which will arrive in JSON format. 
//But somehow this function sometimes behaves oddly, especially when the connection to the server is unreliable or slow......
function getData(){
